(ns vega-vis.core
  (:require
    ...
    [reagent.dom.server :as rcs]
    [vega-tools.core :as vt]
    [promesa.core :as p]

    [vega-vis.helpers :as h]
    [vega-vis.stocks-index-chart :as sic]
    ))

;; todo: include vega-lite
;; todo: add multiple elements to the page, e.g. :h1, :h2, :table, etc...
...
(defn app-state-view [as]
  (let [{:keys [error chart]} @as]
     [:div
      ...
      ;; (type (h/stocks-data))
      ;;"blah"
      ;; (rcs/render-to-sring (h/stocks-data))
      ;; sic/stocks-data
...
(defn on-js-reload []
  ;; (swap! app-state assoc :data (:bar examples))

  ;; Bar-chart
  (swap! app-state assoc :data (->> (h/random-data 50 500)   ;;  <-- random data
                                       (h/bar-chart)))

  ;; Stocks? (not working...)
  ;;  (swap! app-state assoc :data (->> (h/stocks-data)))

  
  (parse-input)
  (reagent/render [app-state-view app-state]
